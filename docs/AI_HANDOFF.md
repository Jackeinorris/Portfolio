...
Last updated **2026-07-09**. The **Teleprompter app is RELEASED on the Microsoft Store** (`https://apps.microsoft.com/detail/9PMJD1Q6Z6L0`); the launch checklist is fully closed, and **UX Cycles 1–3 of the §18 backlog are deployed and verified live**. Latest commit: `bf2921b` "Rework mobile nav" (Cycle 3). **At the time of writing the push of `bf2921b` was still pending** (`master` ahead of `origin/master` by 1) and this handoff file itself is uncommitted (advisory; commit it alone or with the next cycle). **Next session: start with §18 — Cycle 4 (footer wayfinding).** Before acting, re-verify with `git status` / `git log -6 --oneline`. File paths and CSS/build-script line references may drift.
...
- Latest local commit `bf2921b` "Rework mobile nav: stacked header, touch targets, aria-current" (UX Cycle 3, deployed + verified live 2026-07-09). **At last check `master` was ahead of `origin/master` by 1 — confirm the push landed (`git status -sb`), run `git push` if not.** Recent: `79e5812` UX Cycle 2 (prev/next on projects), `9e51c97` UX Cycle 1 (product strip + nav item), `8c01e89` handoff docs, `264b7c3` Store CTA swap.
...
- **UX backlog — active workstream, see §18** (Cycles 1–3 done and live; Cycle 4 = footer wayfinding is next).
...
| 4 | **Footer wayfinding: echo nav links (Projetos/Ensaios/Teleprompter) in the footer — same ~11-file churn + `renderFooter()` template in the build script. Mind `aria-current` if links mirror the nav (§7).** | NEXT |
| — | Minor: consider renaming "Arquivo" section; product-page screenshots (needs user material) | unscheduled |

Notes: nav/footer centralization was **decided against** on 2026-07-09 (see §17) — Cycle 4 proceeds as a mechanical multi-file edit. Any nav-touching edit must respect the per-page `aria-current` (§7).
